...
function toJSON(value, opts = {
  ...
  const iterable = value instanceof Map ? [...value.entries()] : entries(value);
  return iterable.reduce((acc, [k, v]) => {
    if (v instanceof Map) {
      acc[k] = toJSON(v);
...
function fromJSON(...args) {
  ...
  return new Map(iterable.map(([k, v]) => {
    if (v instanceof Map) {
      return [k, fromJSON(v)];
    }
...
function merge(value, props) {
  if (typeof props === 'object' && props.constructor === Object) {
    props = entries(props);
  } else if (props instanceof Map) {} else {
    return value;
  }
...
function getIn(value, path) {
  return toPath(path).reduce((acc, val) => acc.has(val) ? acc.get(val) : acc, value);
}
